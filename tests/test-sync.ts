...
function assertIncludes(arr: string[], item: string, label: string): void {
	if (arr.includes(item)) {
		...
		console.error(
			`  FAIL: ${label} — ${JSON.stringify(item)} not in ${JSON.stringify(arr)}`,
		);
		FAIL++;
	}
}
...
import type { PackagesConfig, RhoConfig } from "../cli/config.ts";
...
type ModulesConfig = RhoConfig["modules"];

type ManagedRhoPackage = {
	source?: unknown;
	_managed_by?: unknown;
	extensions?: string[];
};
...
function makeConfig(overrides?: {
	...
	if (overrides?.modules) {
		for (const category of Object.keys(overrides.modules) as Array<
			keyof ModulesConfig
		>) {
			const moduleOverrides = overrides.modules[category];
			if (!moduleOverrides) continue;
			Object.assign(modules[category], moduleOverrides);
		}
	}
	return {
		agent: { name: "test" },
...
function expectArray<T>(value: T[] | undefined, label: string): T[] {
...
console.log("\n-- buildRhoPackageEntry: one tool disabled --");
{
	...
	const extensions = expectArray(entry.extensions, "extensions array present");
	assertIncludes(
		extensions,
		"extensions/**/*.ts",
...
);
{
	...
	assertIncludes(
		...
		"excludes rho-cloud-email skill",
	);
	assertIncludes(
		skills,
		"!skills/rho-cloud-onboard",
		"excludes rho-cloud-onboard skill",
	);
}
...
console.log("\n-- buildRhoPackageEntry: multiple modules disabled --");
{
	...
	assertIncludes(extensions, "!extensions/x-search/**", "excludes x-search");
	assertIncludes(extensions, "!extensions/email/**", "excludes email");
	assertIncludes(
		extensions,
		"!extensions/vault-search/**",
		"excludes vault-search",
	);

	// Skills
	const skills = expectArray(entry.skills, "skills array present");
	assertIncludes(skills, "skills/*", "starts with skills/*");
	assertIncludes(skills, "!skills/rho-cloud-email", "excludes rho-cloud-email");
	assertIncludes(
		skills,
		"!skills/rho-cloud-onboard",
		"excludes rho-cloud-onboard",
	);
	assertIncludes(skills, "!skills/vault-clean", "excludes vault-clean");
}
...
console.log("\n-- buildRhoPackageEntry: core modules cannot be disabled --");
{
	...
	if (entry.extensions) {
		assertNotIncludes(
			entry.extensions,
			"!extensions/rho",
			"core heartbeat extension not excluded",
		);
		assertNotIncludes(
			entry.extensions,
			"!extensions/memory-viewer",
			"core memory extension not excluded",
		);
	} else {
	...
	if (entry.skills) {
		assertNotIncludes(
			...
			"core memory-consolidate skill not excluded",
		);
	} else {
...
{
	const config = makeConfig({
		modules: { skills: { "session-search": false } },
	});
	const entry = buildRhoPackageEntry(config, RHO_ROOT);
...
console.log("\n-- buildRhoPackageEntry: workflows skill module disabled --");
{
	...
	assert(
		...
		"workflows disable has no extension exclusions",
	);
	const skills = expectArray(entry.skills, "skills array present");
	assertIncludes(skills, "!skills/pdd", "excludes pdd skill");
	assertIncludes(skills, "!skills/code-assist", "excludes code-assist skill");
}
...
console.log("\n-- buildRhoPackageEntry: all non-core modules disabled --");
{
	...
	const extExclusions = extensions.filter((p) => p.startsWith("!"));
...
console.log("\n-- findRhoEntryIndex: find by source path --");
{
	...
	assertEq(
		findRhoEntryIndex(packages, RHO_ROOT),
		1,
		"finds by source path at index 1",
	);
}
...
console.log("\n-- findRhoEntryIndex: find bare string entry --");
{
	...
	assertEq(
		findRhoEntryIndex(packages, RHO_ROOT),
		1,
		"finds bare string at index 1",
	);
}

console.log("\n-- findRhoEntryIndex: not found --");
{
	const packages = ["npm:pi-interactive-shell", "../../projects/pi-ralph"];
...
console.log("\n-- buildSyncLock: tracks package sources --");
{
	...
	assertEq(
		lock.managed_packages,
		["npm:some-package", "git:github.com/user/repo"],
		"tracks all sources",
	);
}
...
console.log("\n-- planSync: fresh install (no existing settings) --");
{
	...
	assert(
		plan.settingsJson.packages.length >= 1,
		"settings has at least the rho package",
	);
}
...
console.log("\n-- planSync: upgrades bare string to object form --");
{
	...
	const rhoIdx = plan.settingsJson.packages.findIndex(
		(pkg: unknown) =>
			typeof pkg === "object" &&
			pkg !== null &&
			(pkg as ManagedRhoPackage)._managed_by === "rho",
	);
	...
	assertEq(
		plan.settingsJson.packages[0],
		"npm:pi-interactive-shell",
		"first package preserved",
	);
	assertEq(
		plan.settingsJson.packages[2],
		"../../projects/pi-ralph",
		"third package preserved",
	);
	// Verify non-packages fields preserved
	assertEq(
		plan.settingsJson.defaultProvider,
		"anthropic",
...
console.log("\n-- planSync: updates existing object entry --");
{
	const existingSettings = {
		packages: [
			"npm:pi-interactive-shell",
			{
				source: RHO_ROOT,
				_managed_by: "rho",
				extensions: ["extensions/**/*.ts", "!extensions/old"],
			},
		],
	};
	const config = makeConfig({ modules: { tools: { "x-search": false } } });
	...
	const rhoEntry = plan.settingsJson.packages[1] as ManagedRhoPackage;
	assertEq(rhoEntry._managed_by, "rho", "marker preserved");
	const rhoExtensions = expectArray(
		rhoEntry.extensions,
		"rho entry has extensions array",
	);
	assertIncludes(
		rhoExtensions,
		"!extensions/x-search/**",
	...
	assertNotIncludes(rhoExtensions, "!extensions/old", "old exclusion removed");
}
...
console.log("\n-- planSync: existing packages.toml entry already installed --");
{
	...
	const existingSettings = {
		packages: [{ source: RHO_ROOT, _managed_by: "rho" }, "npm:cool-ext"],
	};
	const plan = planSync({
		config,
		pkgConfig,
...
console.log("\n-- planSync: package removed from packages.toml --");
{
	...
	const existingSettings = {
		packages: [{ source: RHO_ROOT, _managed_by: "rho" }, "npm:old-pkg"],
	};
	...
	assertEq(
		plan.packagesToRemove,
		["npm:old-pkg"],
		"old package flagged for removal",
	);
	// Should remove from settings.json packages array
	const sources = plan.settingsJson.packages
		.map((pkg: unknown) => packageSource(pkg))
		.filter((source): source is string => source !== null);
	assertNotIncludes(
		sources,
		"npm:old-pkg",
		"removed package not in settings.json",
	);
}
...
console.log("\n-- planSync: does not remove unmanaged packages --");
{
	...
	const pkgConfig = makeEmptyPackages();
	const existingSettings = {
		packages: [{ source: RHO_ROOT, _managed_by: "rho" }, "npm:user-installed"],
	};
...
console.log("\n-- planSync: sync lock generated --");
{
	...
	assertEq(
		plan.newSyncLock.managed_packages,
		["npm:pkg-a", "npm:pkg-b"],
		"lock tracks all packages",
	);
