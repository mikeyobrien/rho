...

{
	...
	const tmpDir = fs.mkdtempSync(
		path.join(process.env.TMPDIR || "/tmp", "rho-test-path-"),
	);
...

{
	...
	const cmdLineIdx = lines.findIndex(
		(l) =>
			l.includes("const command") &&
			l.includes("pi") &&
			l.includes("--no-session") &&
			l.includes("HEARTBEAT_PROMPT_FILE"),
	);
	...
	assertMatch(
		fnSlice,
		/heartbeatPaneDead[\s\S]*?\|\|[\s\S]*?heartbeatPaneBusy|heartbeatPaneBusy[\s\S]*?\|\|[\s\S]*?heartbeatPaneDead/,
...

{
	...
	if (startIdx >= 0) {
		...
		const sessionPos = sessionFallbackMatch
			? fnSlice.indexOf(sessionFallbackMatch[0])
			: -1;

		assert(pinnedPos >= 0, "#9: buildModelFlags checks pinned model");
		assert(
			autoResolvePos >= 0,
			"#9: buildModelFlags calls resolveHeartbeatModel",
		);
		assert(
			sessionPos >= 0,
			"#9: buildModelFlags checks session model (if ctx.model)",
		);
