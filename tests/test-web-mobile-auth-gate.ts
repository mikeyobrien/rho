...
async function runTests() {
	...
	console.log("\n-- Route-level tests --");
	{
		...
		const resStatus = await app.fetch(new Request("http://localhost/api/auth/status"));
		...
		const resInvalid = await app.fetch(new Request("http://localhost/api/config", {
		...
		const resValid = await app.fetch(new Request("http://localhost/api/config", {
	...
	console.log("\n-- WS handshake tests --");
	{
		...
		// Allowed with auth
		const resWsAllowed = await app.fetch(new Request("http://localhost/ws", {
			headers: validAuthHeader
		}));
