...
let taskCache: { mtimeMs: number; tasks: TaskEntry[] } | null = null;

async function loadBrainTasks(): Promise<TaskEntry[]> {
	let mtimeMs = 0;
	try {
...
export async function createTask(params: {
	...
	taskCache = null; // invalidate cache after write
...
export async function updateTask(
	...
	const task = tasks.find(
		(t) => t.id === id.trim() || t.id.startsWith(id.trim()),
	);
