...
const listEl = requiredEl("profile-list");
const modalEl = requiredEl("modal");
const modalTitleEl = requiredEl("modal-title");
const launchBtn = requiredEl("btn-launch");
const addBtn = requiredEl("btn-add");
const saveBtn = requiredEl("btn-save");
const cancelBtn = requiredEl("btn-cancel");
...
const liveModeToggleBtn = document.getElementById("btn-live-mode");
const liveModeStatusEl = document.getElementById("live-mode-status");
const liveModeToggleBtnPicker = document.getElementById("btn-live-mode-picker");
const liveModeStatusElPicker = document.getElementById(
	"live-mode-status-picker",
);
// Form elements
const inputId = requiredEl("input-id");
const inputName = requiredEl("input-name");
const inputScheme = requiredEl("input-scheme");
const inputHost = requiredEl("input-host");
const inputPort = requiredEl("input-port");
const inputToken = requiredEl("input-token");
function shouldUseEmbeddedFrame(url) {
...
function renderProfiles() {
	...
	if (currentProfiles.length === 0) {
		listEl.innerHTML =
			'<p class="profile-empty">No profiles found. Add one to get started.</p>';
	...
	for (const profile of currentProfiles) {
		...
		const badgeClass =
			profile.scheme === "https"
				? "scheme-badge scheme-badge--https"
				: "scheme-badge scheme-badge--http";
...
function updateLaunchButton() {
	const selectedExists =
		!!selectedProfileId &&
		currentProfiles.some((p) => p.id === selectedProfileId);
	launchBtn.disabled = !selectedExists;
}
function generateId() {
	return Math.random().toString(36).substring(2, 9);
}
async function openModal(profile) {
	if (profile) {
...
async function saveProfile() {
	const profile = {
		...
		port: Number.parseInt(inputPort.value, 10) || 0,
	};
...
function updateLiveModeUI() {
	...
	const statusEls = [liveModeStatusEl, liveModeStatusElPicker].filter(Boolean);
	const toggleBtns = [liveModeToggleBtn, liveModeToggleBtnPicker].filter(
		Boolean,
	);
	...
	for (const btn of toggleBtns) {
		btn.textContent =
			state === "live" || state === "starting" ? "Stop Live" : "Go Live";
...
if (switchProfileBtn) {
	...
	console.warn(
		"[rho-android] #btn-switch-profile not found; profile picker remains functional.",
	);
}
...
function isPickerModeRequested() {
	try {
...
document.addEventListener("DOMContentLoaded", async () => {
	try {
		...
		console.error("Failed to initialize rho-android shell", error);
		alert(`Failed to initialize app shell: ${getErrorMessage(error)}`);
	}
});
