...
function readAuth(): PiAuthData | null {
	return readPiAuth();
}

async function fetchCodexUsage(token: string): Promise<UsageState["codex"]> {
	const summary = await fetchCodexUsageSummary(token);
	...
	return {
		session: summary.primaryUsedPercent ?? 0,
		weekly: summary.secondaryUsedPercent ?? 0,
	};
}
...
function detectProvider(model: unknown): "codex" | "claude" | null {
	...
	const p = (candidate.provider || "").toLowerCase();
	const id = (candidate.id || "").toLowerCase();
	const name = (candidate.name || "").toLowerCase();
	const api = (candidate.api || "").toLowerCase();
	...
	if (
		(p.includes("google") || p.includes("antigravity")) &&
		(id.includes("claude") || name.includes("claude"))
	) {
		return "claude";
	}
...
export default function (pi: ExtensionAPI) {
	...
	function updateProviderFrom(modelLike: unknown): boolean {
